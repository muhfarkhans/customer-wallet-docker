<div x-on:refresh-5.window="setTimeout(() => $dispatch('refresh-all'), 5000)">
    <x-primary-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'form-deposit')"
        class="w-full bg-teal-500">
        {{ __('Deposit') }}
    </x-primary-button>

    <x-modal name="form-deposit" :show="$errors->isNotEmpty()" focusable>
        <form wire:submit="store" class="p-6">
            ...
            <div class="flex justify-end mt-6">
                ...
                <x-primary-button class="bg-teal-500 ms-3">
                    {{ __('Deposit') }}
                </x-primary-button>
            </div>
        </form>
